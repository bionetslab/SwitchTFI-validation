...
def main_endocrine():
    ...
    for offspring in offspring_names:
        ...
        data_pipeline(adata=adata.copy(),
                      res_path=res_p,
...
def main_hematopoiesis():
    ...
    def rename_mus_musculus_hematopoiesis_cells(adata: sc.AnnData,
                                                cluster_obs_key: str = 'paul15_clusters') -> sc.AnnData:
        name_dict = {'19Lymph': 'lymphocytes_19',
        ...
        cell_types = adata.obs[cluster_obs_key].to_numpy()
        dummy = np.empty(adata.n_obs, dtype=object)
        for key, item in name_dict.items():
            dummy[cell_types == key] = item

        adata.obs[f'{cluster_obs_key}_renamed'] = dummy
        dummy2 = np.empty(adata.n_obs, dtype=object)
        cluster_list_dict = {'erythrocytes': ['1Ery', '2Ery', '3Ery', '4Ery', '5Ery', '6Ery'],
        ...
        adata.obs[f'{cluster_obs_key}_coarse'] = dummy2

        return adata

    adata = rename_mus_musculus_hematopoiesis_cells(adata=adata, cluster_obs_key='paul15_clusters')
    ...
    for key, val in prog_off_dict.items():
        dummy[np.isin(cell_types, val)] = key
    adata.obs['prog_off'] = dummy
